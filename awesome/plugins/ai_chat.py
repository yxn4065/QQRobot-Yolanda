...
EXPR_DONT_UNDERSTAND = (
    '>小yo现在还不太明白你在说什么呢，但没关系，以后的我会变得更强呢！',
    '>小yo有点看不懂你的意思呀，可以跟我聊些简单的话题嘛',
    '>其实小yo不太明白你的意思……',
    '>抱歉哦，小yo现在的能力还不能够明白你在说什么，但我会加油的～'
)
...
async def call_tencent_bot_api(session: CommandSession, text: Optional[str]) -> Optional[str]:
    ...
    try:
        ...
        httpProfile.endpoint = "nlp.tencentcloudapi.com"  # 默认就近接入

        # 实例化一个client选项，可选的，没有特殊需求可以跳过
        clientProfile = ClientProfile()
        clientProfile.httpProfile = httpProfile

        # 实例化要请求产品nlp的client对象，clientProfile是可选的。
        ...
        # 实例化一个nlp实例信息查询请求对象,每个接口都会对应一个request对象。
        req = models.ChatBotRequest()
        req.from_json_string(json.dumps(params))

        # 通过client对象调用ChatBot发起请求,注意请求方法名与请求对象是对应的
        resp = client.ChatBot(req).to_json_string()
        print("response:", resp)  # 输出json格式的字符串回包
        resp_payload = json.loads(resp)
        # print(resp_payload.get('Reply'))# 查看接口返回信息
